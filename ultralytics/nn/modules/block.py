...
class CSPMirrorNet(nn.Module):
   ...
   def __init__(self,num_of_base_blocks:int,input_shape:Tensor,verbose:bool=False,overlap_percentage:float=0.20,stride:int=1)->None:
      ...
      print('out channels', num_of_base_blocks)
   ...
   def forward(self, feature_map:Tensor)->Tensor: #also can be an image/Feaute map
      ...
      if(example1.shape[1]!=processed_part1.shape[1]):
        if example1.shape[1] > processed_part1.shape[1]:
            ...
            for i in range(1, num_chunks):
                reduced_example1 += example1[:, i * processed_part1.shape[1]:(i + 1) * processed_part1.shape[1], :, :]

            # Divide by number of chunks to normalize
            example1 = reduced_example1 / num_chunks


            
            reduced_example2 = example2[:, :processed_part1.shape[1], :, :]

            for i in range(1, num_chunks):
                reduced_example2 += example2[:, i * processed_part1.shape[1]:(i + 1) * processed_part1.shape[1], :, :]
      ...
      concat2 = example1 + processed_part2 #torch.cat([example1, processed_part2], dim=1)
